...
func RegisterAfterAuthenticateDevice(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, out *api.Session, in *api.AuthenticateDeviceRequest) error {
	logger.WithFields(u.Inject(ctx, b3.B3MultipleHeader)).WithFields(map[string]interface{}{"name": "RegisterAfterAuthenticateDevice", "in": in, "out": out}).Debug("")
	...
	if err := u.Redpanda(ctx, logger, map[string]interface{}{"name": "RegisterAfterAuthenticateDevice", "in": in, "out": out}); err != nil {
