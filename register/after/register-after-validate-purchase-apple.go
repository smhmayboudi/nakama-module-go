...
func RegisterAfterValidatePurchaseApple(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, out *api.ValidatePurchaseResponse, in *api.ValidatePurchaseAppleRequest) error {
	logger.WithFields(u.Inject(ctx, b3.B3MultipleHeader)).WithFields(map[string]interface{}{"name": "RegisterAfterValidatePurchaseApple", "in": in, "out": out}).Debug("")
	...
	if err := u.Redpanda(ctx, logger, map[string]interface{}{"name": "RegisterAfterValidatePurchaseApple", "in": in, "out": out}); err != nil {
